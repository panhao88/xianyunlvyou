    ...
    <div class="box2">
      ...
      <div v-if="didi === 0">
        ...
        <a-form ref="ruleForm" :model="form" :rules="rules" >
          ...
          <a-form-item>
            <a-button type="primary" @click="denglu">登录</a-button>
          </a-form-item>
        </a-form>
      </div>
      <div v-if="didi === 1">
        <a-form ref="ruleForm1" :model="form1" :rules="rules1">
          ...
          <a-form-item name="password">
            <a-input-password v-model:value="form1.password" placeholder="请输入密码" />
          </a-form-item>
          <a-form-item name="confirm">
            <a-input-password v-model:value="form1.confirm" placeholder="确认密码" />
          </a-form-item>
          <div>
...
import { Code, Resyanzheng, Des ,} from "@/types";
    ...
    const yanzhengma = (): void => {
      api
        ...
        .then((res: Resyanzheng) => {
          ...
          if (data.Code) {
            message.success(`验证码：${data.Code.code}`);
          }
          ...
          if (data.form1.password !== data.form1.confirm) {
            ...
            api
              ...
              .then((res: Des) => {
                ...
                localStorage.setItem("user", JSON.stringify(data.haha.user));
                router.push("/");
              })
              .catch((err: any) => {
                console.log(err);
