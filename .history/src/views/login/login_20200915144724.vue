  ...
  <div class="box1">
    ...
    <div class="box2">
      <div style="display: flex;justify-content: space-around;height:60px; align-items: center; background: gray;">
        <div :class="didi === 0 ? 'color':''" @click="godenglu(item,index)">登录</div>
        <div :class="didi === 1 ? 'color':''" @click="gozhuce(item,index)">注册</div>
      </div>
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  setup(props, ctx: SetupContext) {
    ...
    let godenglu = (item:number,index:number) => {
      data.didi = 0
    },
    let gozhuce = (item:number,index:number) => {
      data.didi = index
    }
    return {
      ...toRefs(data),
