...
import { Code, Resyanzheng } from '@/types'
//轮播图请求验证
interface Res {
  data: dataitem[];
  total?: number;
  ...
  setup(props, ctx: SetupContext) {
    ...
    const yanzhengma = ():void => {
      api.getcaptchas({
        ...
        data.form1.captcha = data.Code.code! as str
        console.log(res)
      }).catch((err:any)=>{
        console.log(err)
      })
    ...
    //注册
    const zhuce=(): void=>{
     ruleForm1.value!.validate().then(()=>{
       api.getzhuce({username: data.form1.username,nickname: data.form1.nickname,captcha: data.form1.captcha,password: data.form1.password}).then((res: any)=>{
       console.log(res);
