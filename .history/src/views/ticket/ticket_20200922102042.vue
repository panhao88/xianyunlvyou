...
export default defineComponent({
  ...
  setup(props, ctx: SetupContext) {
    ...
    const search =() :void => {
      router.push({path:'jipiao',query:{chufa:data.chufa,daoda:data.daoda,day:data.day}})
    }
    const yyyy1 = ():void => {
       //机票出发
        api.getairscity1({name:data.chufa}).then((res:any) => {
      ...
      const yyyy2 = ():void => {
       //机票出发
        api.getairscity1({name:data.daoda}).then((res:any) => {
          console.log(res ,"出发城市")
        }).catch((err) => {
          console.log(err)
        })
