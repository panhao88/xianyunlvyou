    ...
    <div class="box1">
      <div class="box2">
        <div style="width:1050px;">
            ...
            <div>
              ...
              >
                ...
                :value="item"
                  ...
                  :key="index"
                ><div @click="handleChange1(item)">{{item}}</div></a-select-option>
              </a-select>
            </div>
            <div>
              <a-select
              ...
              >
                ...
                :value="`${item.from}:00 - ${item.to}:00`"
                  ...
                  :key="index"
                ><div @click="handleChange2(item.from)">{{item.from}}-00 {{item.to}}-00</div></a-select-option>
              </a-select>
            </div>
            <div>
              <a-select
              ...
              >
                <a-select-option :value="item" v-for="(item,index) in options.company" :key="index" >
                  <div @click="handleChange3(item)">{{item}}</div>
                </a-select-option>
              </a-select>
            </div>
            <div>
              <a-select
                ...
                placeholder="机型"
              >
                <a-select-option :value="item.size" v-for="(item,index) in jixing" :key="index">
                  <div  @click="handleChange4(item.size)" >{{item.didi}}</div>
                </a-select-option>
              </a-select>
            </div>
          </div>
          ...
          <div class="m-t-2">
            <div
              class="b-r"
              v-for="(item,index) in didi1.slice((current-1)*pageSize,current*pageSize)"
              :key="index"
      ...
      <div style="margin-top:10px">
        <a-pagination
          ...
          show-size-changer
          show-quick-jumper
          :show-total="total => `共 ${didi1.length} 条`"
        >
...
export default defineComponent({
  ...
  setup(props, ctx: SetupContext) {
    ...
    //起飞机场
    const handleChange1 = (value: string): void => {
    ...
    const handleChange2 = (value: string): void => {
      ...
      data.didi1 = flights222!;
    };
    //起飞航空
    const handleChange3 = (value:string) :void => {
      data.didi1 = data.flights
      const didi2 = []! as flightsItem[];
      const didi3 = data.didi1
      didi3.map(item => {
    ...
    // 起飞机型
     const handleChange4 = (value:string) :void => {
      data.didi1 = data.flights
      const didi2 = []! as flightsItem[];
      const didi3 = data.didi1
      didi3.map(item => {
    ...
    onMounted(() => {
      ...
      api
        ...
        .then((res: any) => {
          ...
          data.didi1 = data.flights
           console.log(data.didi1,11);
           console.log(data.flights,22);
