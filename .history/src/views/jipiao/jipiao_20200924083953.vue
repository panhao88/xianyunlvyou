          ...
          <div class="d-f j-c-b m-t-2">
            ...
            <div>
              <a-select
                ...
                placeholder="航空公司"
              >
                <a-select-option :value="item" v-for="(item,index) in options.company" :key="index" @click="handleChange2(item)">{{item}}</a-select-option>
              </a-select>
            </div>
            <div>
              <a-select
...
export default defineComponent({
  ...
  setup(props, ctx: SetupContext) {
    ...
    //起飞时间
    //起飞航空
    const handleChange2 = (value3:string) :void => {
      data.didi2 = data.flights
      const didi1 = []! as flightsItem[];
      const didi3 = data.didi2
      didi3.map(item => {
       if(item.airline_name === value3)
       didi1.push(item)
      })
      data.flights = didi1
    }
