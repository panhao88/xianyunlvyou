      ...
      <div class="box2">
        ...
        <div class="bot1">
          ...
          <div style="width:1050px;display:flex;justify-content: space-between;">
            <div class="f-c-hei f-s " style="margin-top:20px;color:orange;border-bottom:solid 2px orange;padding-bottom">推荐攻略</div>
          ...
          <div class="boto">
            <div>
              <div v-for="(item,index) in posts.slice(0,4)" :key="index">
                <div class="bott"></div>
                <div class="f-s-2 f-c-hei" @click="goto(item)" @mouseenter="baba(index)" @mouseleave="mama(index)" :class="caonima === index ? 'yanse':''">{{item.title}}</div>
                <div class="bott1 f-c-hei" @click="goto(item)">{{item.summary}}</div>
                <div class="d-f">
                  <div v-for="(item1,index1) in item.images.slice(0,3)" :key="index1" class="p-r">
                    <div>
                      <img :src="item1" alt style="width:300px;height:200px;" @click="goto(item)"/>
                    </div>
                  </div>
                </div>
...
export default defineComponent({
  ...
  setup(props, ctx: SetupContext) {
    ...
    onMounted(() => {
      data.id = route.query.id! as number
      //城市列表
      api
        .getcities()
        .then((res: tuyo) => {
